// ============================================================================
// PRAGYAN INSTITUTE — SERVERLESS PUSH BROADCAST DISPATCHER
// ----------------------------------------------------------------------------
// POST /api/send-push
// Protected by Admin JWT Session or Bearer CRON_SECRET.
// Dispatches encrypted W3C Web Push (RFC 8291/8292) notifications to
// registered student devices with dynamic variable interpolation and
// automatic 404/410 dead-subscription pruning.
// ============================================================================
...
export default async function handler(req, res) {
  ...
  // Auth: either Admin JWT Session OR Cron Secret
  let session = null;
  ...
  if (!isCron) {
    ...
    if (!session) return; // requireSession answers 401/403
  }
  ...
  try {
    // 1. Fetch target subscriptions and mapped student profiles
    ...
    // 3. Dispatch in concurrency waves of 10
    const CHUNK_SIZE = 10;
    for (let i = 0; i < targetSubs.length; i += CHUNK_SIZE) {
      ...
      const promises = chunk.map(async (sub) => {
        ...
        const res = await pushToSubscription(sub, payloadObj, {
          vapidKeys,
          vapidSubject,
          ttlSeconds,
          urgency: priority
    ...
    // 4. Prune dead endpoints from Supabase
