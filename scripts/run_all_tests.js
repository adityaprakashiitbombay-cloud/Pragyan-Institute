...
// -----------------------------------------------------------------------------
// T18: Complete Academic Batches, Special English & Class Code Resolution Suite
// -----------------------------------------------------------------------------
console.log('\n--- [T18] Complete Academic Batches, Special English & Class Code Resolution Suite ---');

function getClassCodeTest(className = '') {
  const cStr = String(className || '').toUpperCase();
  if (cStr.includes('12') || cStr.includes('XII') || cStr.includes('TARGET 12')) return '12';
  if (cStr.includes('11') || cStr.includes('XI') || cStr.includes('TARGET 11')) return '11';
  if (cStr.includes('10') || cStr.includes('ACHIEVER') || cStr.includes('X') || cStr.includes('BOARD') || cStr.includes('MATRIC')) return '10';
  if (cStr.includes('9') || cStr.includes('NURTURE') || cStr.includes('IX')) return '09';
  if (cStr.includes('8') || cStr.includes('ALPHA') || cStr.includes('VIII')) return '08';
  if (cStr.includes('7') || cStr.includes('VII')) return '07';
  if (cStr.includes('6') || cStr.includes('VI')) return '06';
  if (cStr.includes('5') || cStr.includes('V') || cStr.includes('1ST TO 5TH') || cStr.includes('1-5') || cStr.includes('PRIMARY') || cStr.includes('JUNIOR')) return '05';
  if (cStr.includes('4') || cStr.includes('IV')) return '04';
  if (cStr.includes('3') || cStr.includes('III')) return '03';
  if (cStr.includes('2') || cStr.includes('II')) return '02';
  if (cStr.includes('1') || cStr.includes('IST') || cStr.includes('1ST')) return '01';
  const match = cStr.match(/\b([1-9]|1[0-2])\b/);
  if (match) return match[1].padStart(2, '0');
  return '10';
}

assert(getClassCodeTest('Class 12th PCM (Target 12th Board & JEE)') === '12', 'T18.1: Correctly maps Class 12th PCM to CC=12');
assert(getClassCodeTest('Class 12th PCB (Target 12th Board & NEET)') === '12', 'T18.2: Correctly maps Class 12th PCB to CC=12');
assert(getClassCodeTest('Class 11th PCM (I.Sc. Foundation)') === '11', 'T18.3: Correctly maps Class 11th PCM to CC=11');
assert(getClassCodeTest('Class 11th PCB (I.Sc. Foundation)') === '11', 'T18.4: Correctly maps Class 11th PCB to CC=11');
assert(getClassCodeTest('Class 10th (ACHIEVER / Matric Board)') === '10', 'T18.5: Correctly maps Class 10th to CC=10');
assert(getClassCodeTest('Class 9th (NURTURE / Foundation)') === '09', 'T18.6: Correctly maps Class 9th to CC=09');
assert(getClassCodeTest('Class 8th (ALPHA / Middle School)') === '08', 'T18.7: Correctly maps Class 8th to CC=08');
assert(getClassCodeTest('Class 6th & 7th (PIONEER Foundation)') === '07', 'T18.8: Correctly maps Class 6th & 7th to CC=07');
assert(getClassCodeTest('Class 1st to 5th (Junior Foundation)') === '05', 'T18.9: Correctly maps Class 1st to 5th to CC=05');

const mock12Batches = [
  { id: 'BAT-12PCM', name: 'Class 12th PCM', monthlyFee: 1500 },
  { id: 'BAT-12PCB', name: 'Class 12th PCB', monthlyFee: 1500 },
  { id: 'BAT-11PCM', name: 'Class 11th PCM', monthlyFee: 1500 },
  { id: 'BAT-11PCB', name: 'Class 11th PCB', monthlyFee: 1500 },
  { id: 'BAT-10', name: 'Class 10th (ACHIEVER)', monthlyFee: 1000 },
  { id: 'BAT-09', name: 'Class 9th (NURTURE)', monthlyFee: 1000 },
  { id: 'BAT-08', name: 'Class 8th (ALPHA)', monthlyFee: 800 },
  { id: 'BAT-67', name: 'Class 6th & 7th (PIONEER)', monthlyFee: 700 },
  { id: 'BAT-15', name: 'Class 1st to 5th (Junior)', monthlyFee: 500 },
  { id: 'BAT-ENG-912', name: 'Special English: Class 9th to 12th', monthlyFee: 1000 },
  { id: 'BAT-ENG-68', name: 'Special English: Class 6th to 8th', monthlyFee: 700 },
  { id: 'BAT-ENG-15', name: 'Special English: Class 1st to 5th', monthlyFee: 500 }
];

assert(mock12Batches.length === 12, 'T18.10: [Master Batch Directory] Exactly 12 active canonical batches configured');
assert(mock12Batches.find(b => b.id === 'BAT-12PCM').monthlyFee === 1500, 'T18.11: Class 12th PCM monthly fee is ₹1,500/mo');
assert(mock12Batches.find(b => b.id === 'BAT-11PCB').monthlyFee === 1500, 'T18.12: Class 11th PCB monthly fee is ₹1,500/mo');
assert(mock12Batches.find(b => b.id === 'BAT-67').monthlyFee === 700, 'T18.13: Class 6th & 7th monthly fee is ₹700/mo');
assert(mock12Batches.find(b => b.id === 'BAT-15').monthlyFee === 500, 'T18.14: Class 1st to 5th monthly fee is ₹500/mo');
assert(mock12Batches.find(b => b.id === 'BAT-ENG-912').monthlyFee === 1000, 'T18.15: Special English 9th-12th monthly fee is ₹1,000/mo');
assert(mock12Batches.find(b => b.id === 'BAT-ENG-68').monthlyFee === 700, 'T18.16: Special English 6th-8th monthly fee is ₹700/mo');
assert(mock12Batches.find(b => b.id === 'BAT-ENG-15').monthlyFee === 500, 'T18.17: Special English 1st-5th monthly fee is ₹500/mo');

// -----------------------------------------------------------------------------
// T19: 10-Day Staggered Billing Cron, Daily 100 - X Quota Guard & Pragyan AI Suite
// -----------------------------------------------------------------------------
console.log('\n--- [T19] 10-Day Staggered Billing Cron, Daily 100 - X Quota Guard & Pragyan AI Suite ---');

// 1. Email Quota Formula & Guard
function calculateEmailQuota(totalSentToday, dailyLimit = 100) {
  const remaining = Math.max(0, dailyLimit - totalSentToday);
  const percent = Math.min(100, Math.round((totalSentToday / dailyLimit) * 100));
  const canSend = (recipientsCount) => (totalSentToday + recipientsCount <= dailyLimit);
  return { dailyLimit, totalSentToday, remaining, percent, canSend };
}

const quotaTest1 = calculateEmailQuota(24, 100);
assert(quotaTest1.remaining === 76, 'T19.1: Correctly calculates 100 - 24 = 76 remaining email slots');
assert(quotaTest1.canSend(10) === true, 'T19.2: Permits sending 10 emails when 76 slots remaining');
assert(quotaTest1.canSend(80) === false, 'T19.3: Blocks sending 80 emails when only 76 slots remaining');

const quotaTest2 = calculateEmailQuota(100, 100);
assert(quotaTest2.remaining === 0, 'T19.4: Reaches 0 quota when 100 emails sent');
assert(quotaTest2.canSend(1) === false, 'T19.5: Strictly blocks all outgoing emails when daily 100 limit reached');

// 2. 10-Day Staggered Rolling Schedule Verification
const rollingScheduleMap = {
  1:  { type: 'billing',  key: '10th_12th', label: 'Class 10th & Class 12th PCM/PCB' },
  2:  { type: 'billing',  key: '9th_11th',  label: 'Class 9th & Class 11th PCM/PCB' },
  3:  { type: 'billing',  key: '8th',       label: 'Class 8th ALPHA' },
  4:  { type: 'billing',  key: '6th_7th',   label: 'Class 6th & 7th PIONEER' },
  5:  { type: 'billing',  key: '1st_5th',   label: 'Class 1st to 5th Junior Foundation' },
  6:  { type: 'billing',  key: 'special_english', label: 'Special English Batches by Aditi Singh' },
  7:  { type: 'reminder', key: '10th_12th', label: 'Class 10th & 12th Unpaid Reminders' },
  8:  { type: 'reminder', key: '9th_11th',  label: 'Class 9th & 11th Unpaid Reminders' },
  9:  { type: 'reminder', key: '6th_8th',   label: 'Class 6th to 8th Unpaid Reminders' },
  10: { type: 'reminder', key: 'all',       label: 'All Batches Final Grace Period Dues Reminder' }
};

assert(rollingScheduleMap[1].type === 'billing' && rollingScheduleMap[1].key === '10th_12th', 'T19.6: Day 1 bills Class 10th & 12th PCM/PCB');
assert(rollingScheduleMap[2].type === 'billing' && rollingScheduleMap[2].key === '9th_11th', 'T19.7: Day 2 bills Class 9th & 11th PCM/PCB');
assert(rollingScheduleMap[3].type === 'billing' && rollingScheduleMap[3].key === '8th', 'T19.8: Day 3 bills Class 8th');
assert(rollingScheduleMap[4].type === 'billing' && rollingScheduleMap[4].key === '6th_7th', 'T19.9: Day 4 bills Class 6th & 7th');
assert(rollingScheduleMap[5].type === 'billing' && rollingScheduleMap[5].key === '1st_5th', 'T19.10: Day 5 bills Class 1st to 5th');
assert(rollingScheduleMap[6].type === 'billing' && rollingScheduleMap[6].key === 'special_english', 'T19.11: Day 6 bills Special English batches');
assert(rollingScheduleMap[10].type === 'reminder' && rollingScheduleMap[10].key === 'all', 'T19.12: Day 10 sends final grace period dues reminders to unpaid students');

// 3. Website 5% Scholarship Calculation for all 12 batches
const pricingTable = {
  class12pcm: { monthly: 1500, annual: 1500 * 12 * 0.95 },
  class12pcb: { monthly: 1500, annual: 1500 * 12 * 0.95 },
  class11pcm: { monthly: 1500, annual: 1500 * 12 * 0.95 },
  class11pcb: { monthly: 1500, annual: 1500 * 12 * 0.95 },
  class10:    { monthly: 1000, annual: 1000 * 12 * 0.95 },
  class9:     { monthly: 1000, annual: 1000 * 12 * 0.95 },
  class8:     { monthly: 800,  annual: 800 * 12 * 0.95 },
  class67:    { monthly: 700,  annual: 700 * 12 * 0.95 },
  class15:    { monthly: 500,  annual: 500 * 12 * 0.95 },
  eng912:     { monthly: 1000, annual: 1000 * 12 * 0.95 },
  eng68:      { monthly: 700,  annual: 700 * 12 * 0.95 },
  eng15:      { monthly: 500,  annual: 500 * 12 * 0.95 }
};

assert(pricingTable.class12pcm.annual === 17100, 'T19.13: Class 12th annual lump sum is ₹17,100 (5% off ₹18,000)');
assert(pricingTable.class10.annual === 11400, 'T19.14: Class 10th annual lump sum is ₹11,400 (5% off ₹12,000)');
assert(pricingTable.class67.annual === 7980, 'T19.15: Class 6th & 7th annual lump sum is ₹7,980 (5% off ₹8,400)');
assert(pricingTable.class15.annual === 5700, 'T19.16: Class 1st to 5th annual lump sum is ₹5,700 (5% off ₹6,000)');
